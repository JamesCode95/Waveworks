...
HRESULT DXUTSnapD3D10Screenshot( LPCTSTR szFileName )
{
	...
	if (!pSwap) return E_FAIL;

	ID3D10Texture2D* pBackBuffer;
	HRESULT hr = pSwap->GetBuffer(0, __uuidof(ID3D10Texture2D), (LPVOID*)&pBackBuffer);
	...
	ID3D10Texture2D *pCompatableTexture = pBackBuffer;

	if ( dsc.SampleDesc.Count > 1) 
	{
		D3D10_TEXTURE2D_DESC dsc_new = dsc;
		dsc_new.SampleDesc.Count = 1;
		dsc_new.SampleDesc.Quality = 0;
		dsc_new.Usage = D3D10_USAGE_DEFAULT;
		dsc_new.BindFlags = 0;
		dsc_new.CPUAccessFlags = 0;
		ID3D10Texture2D *resolveTexture;
		hr = pDevice->CreateTexture2D(&dsc_new, NULL, &resolveTexture);
		pDevice->ResolveSubresource(resolveTexture, 0, pBackBuffer, 0, dsc.Format);
		pCompatableTexture = resolveTexture;
		pCompatableTexture->GetDesc(&dsc);
	}


	hr = D3DX10SaveTextureToFileW(pCompatableTexture, D3DX10_IFF_BMP, szFileName); 
