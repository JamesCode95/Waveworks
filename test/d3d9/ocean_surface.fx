...
#define GFSDK_WAVEWORKS_DECLARE_ATTR_VS_SAMPLER(Label,TextureLabel,Regoff) sampler Label;
#define GFSDK_WAVEWORKS_DECLARE_ATTR_PS_CONSTANT(Type,Label,Regoff) Type Label;
#define GFSDK_WAVEWORKS_DECLARE_ATTR_PS_SAMPLER(Label,TextureLabel,Regoff) sampler Label;
...
// Foam diffuse map (bubbles)
sampler g_samplerFoamDiffuseMap =
sampler_state
{
    texture = (g_texFoamDiffuseMap);
    MipFilter = Linear;
    MinFilter = Linear;
    MagFilter = Linear;
...
// Foam intensity map (Perlin-like)
sampler g_samplerFoamIntensityMap =
sampler_state
{
    texture = (g_texFoamIntensityMap);
    MipFilter = Linear;
    MinFilter = Linear;
    MagFilter = Linear;
...
float4 OceanWavePS(VS_OUTPUT In) : COLOR
{
	...
	float3 ramp = tex1D(g_samplerColorMap, cos_angle).xyz;
	...
	// reflection color
	float3 reflection_color = lerp(g_SkyColor,texCUBE(g_samplerCubeMap, reflected_eye_to_pixel_vector).xyz, ramp.y);
	...
	float foam_intensity_map_lf = 1.0*tex2D(g_samplerFoamIntensityMap, In.world_pos_undisplaced.xy*0.04*float2(1,1)).x - 1.0;

	// high frequency foam map
	float foam_intensity_map_hf = 1.0*tex2D(g_samplerFoamIntensityMap, In.world_pos_undisplaced.xy*0.15*float2(1,1)).x - 1.0;

	// ultra high frequency foam map
	float foam_intensity_map_uhf = 1.0*tex2D(g_samplerFoamIntensityMap, In.world_pos_undisplaced.xy*0.3*float2(1,1)).x;
	...
	float foam_bubbles = tex2D(g_samplerFoamDiffuseMap, In.world_pos_undisplaced.xy).r;
...
float4 DisplayBufferPS(VS_DEBUG_OUTPUT In) : COLOR
{
	...
	//return color;
	return tex2D(g_samplerColorMap, In.tex_coord);
}
